function [Fij, Sij, Eij, ti, tiPN] = fun3DVTFM(varargin)
% FUNCTIONRUNTFM calculates 3D Viscoelastic Traction Force Microscopy metrics
% [Fij, Sij, Eij, ti, tiPN] = functionRunTFM(u, dm, surface, normals,
% materialProps, materialModel) calculates Deformation gradient Fij,
% Cauchy stress Sij, Lagrangian strain Eij,  
...
%                                Non-equlibrium Shear modulus,
%                                Maxwell Element Viscosity]
%   dt: imaging interval
% OUTPUTS
% ------------------------------------------------------------------------
...
[u, dm, surface, normals, materialModel, materialProps, OS, dt] ...
...
% STRESS TENSOR
[Sij, Eij, ~] = calculateSij(Fij,J,materialProps(1:2),materialModel);

% INCLUDE VISCOUS EFFECT
SijV = includeVisco(Fij, materialProps(3), materialProps(4), dt);

% TOTAL STRESS TENSOR
SijTotal = cell(1,1);
for i = 1:length(Sij)
    SijTotal{i} = cellfun(@(x,y) x+y, Sij{i}, SijV{i},'UniformOutput',false);
end

% CALCULATE SURFACE TRACTIONS (Cauchy Relation)
[ti, tiPN] = calculateTractions(SijTotal,surface,normals,OS);
...
for time = 1:timeTotal-1
    % Compute the trial elastic deformation gradient (Eq. 3, pg 6)
    ...
    % Compute the trial Mandel Stress, which is deviatoric (Eq. 5, pg 6)
    ...
    % Compute the viscous stretching (Eq 6, pg 6)
    D_v = cellfun(@(x) 1/(sqrt(2)) * Gamma_v.*x, N_v, 'UniformOutput',false);
    %Viscous deformation gradient ( Eq 7 pg 7)
